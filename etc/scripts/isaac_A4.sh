...
kill_all; OUT=results/claudecodefortest/cert_A4_090; rm -rf $OUT; mkdir -p $OUT; log "A4 START δ0.90 (3.924 N·m)"
...
timeout 3600 python3 -u online_rl_main.py --headless --sweep --sweep-mode torque --capture-mode hijack --log-zu \
  ...
  --capture-biases 3.924 --episodes 2 --speed 2.5 --outdir $OUT > $OUT/run.log 2>&1
log "A4 END rows=$(wc -l < $OUT/sweep_summary.csv 2>/dev/null||echo 0) err=$(grep -c -E 'Traceback|HARD_RESET' $OUT/run.log)"; kill_all; touch results/claudecodefortest/A4_DONE
