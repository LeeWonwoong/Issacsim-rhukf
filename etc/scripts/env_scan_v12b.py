...
A2 = dict(STAGE, REWARD_MODE='A2', A2_P='0.5', A2_C='6.0', A2_C0='0.5', A2_F='1.0', A2_INSTALL='1', GAMMA='0.95', EPS_DECAY='12000')
CELLS = {f'A2dk{k}': dict(A2, A2_DELAY_K=str(k)) for k in (12, 24, 32)}
...
for k in (4, 8, 16):
    for ag in ('Adam', 'SW1'):
        envv, agt = (SW[ag], 'rhukf') if ag != 'Adam' else (AG['Adam'][0], 'adam')
        jobs.append((f'A2dk{k}_{ag}_s43', envv, agt, 43, dict(A2, A2_DELAY_K=str(k))))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v12b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v12b_w{WID}.json', 'w'), default=float)
open(f'{N}/V12b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
