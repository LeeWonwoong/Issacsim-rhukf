...
def run(name, env, ag, sd, extra):
    ...
    t0 = time.time(); h = SR.run_config(env, n_ep=15, seed=sd, ep_steps=300, agent_type=ag, obs_noise=NOISE.get(name.split('_')[0], 0.0))
...
# v26 (09-14, R2): 초기 샘플효율 — greedy 프로브(에피소드마다, ε 스케줄 무관) × 15 에피 × 10 시드. 공정화: Adam He 초기화·Huber β3·AMS off, lr {3e-4,1e-3,3e-3}
FRAME = dict(STAGE, EPS_HOVER_P='0.1', GAMMA='0.9', HOVER_DWELL='5', SURR_V5='1', PROBE_EVERY='1', PROBE_N='6')
CELLS = {'d4000': dict(FRAME, EPS_DECAY='4000'), 'd500': dict(FRAME, EPS_DECAY='500')}
NOISE = {}
...
ADAMP = dict(NET_HIDDEN=16, ADAM_AMSGRAD='0', ADAM_INIT='he', ADAM_HUBER_BETA='3')
JOBSET = os.environ.get('JOBSET', 'gpu')
AG_GPU = {'SW': (SWB, 'rhukf'), 'UKFm': (dict(KTD, RHUKF_MODE='ukf'), 'rhukf'), 'EKFm': (dict(KTD, RHUKF_MODE='ekf'), 'rhukf')}
AG_CPU = {'Adam3e-4': (dict(ADAMP, ADAM_LR='3e-4'), 'adam'), 'Adam1e-3': (dict(ADAMP, ADAM_LR='1e-3'), 'adam'), 'Adam3e-3': (dict(ADAMP, ADAM_LR='3e-3'), 'adam'),
          'AdamRaw3e-4': (dict(NET_HIDDEN=16, ADAM_AMSGRAD='0', ADAM_LR='3e-4'), 'adam')}
AGENTS = AG_GPU if JOBSET == 'gpu' else AG_CPU
...
try: out = json.load(open(f'{N}/v26{JOBSET}_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v26{JOBSET}_w{WID}.json', 'w'), default=float)
open(f'{N}/V26{JOBSET}_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
