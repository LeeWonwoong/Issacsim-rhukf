...
CELLS = {f'A2dk{k}': dict(A2, A2_DELAY_K=str(k)) for k in (0, 2, 4, 8, 16)}; CELLS['A2off'] = dict(A2)
...
try: out = json.load(open(f'{N}/v12_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v12_w{WID}.json', 'w'), default=float)
open(f'{N}/V12_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
