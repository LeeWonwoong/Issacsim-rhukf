...
OUT=results_o3_swirl_winner
for p in $(pgrep -f "python.sh run_sim" 2>/dev/null); do kill -9 $p 2>/dev/null||true; done
for p in $(ps -eo pid,args|grep "isaacsim/kit"|grep -v grep|awk '{print $1}'); do kill -9 $p 2>/dev/null||true; done
sleep 5
rm -rf $OUT; mkdir -p $OUT
echo "[$(date '+%m-%d %H:%M')] swirl_winner START" | tee -a isaac_winner.log
env NET_HIDDEN=16 RHUKF_N=5 RHUKF_Q=1e-2 RHUKF_TAU=0.005 RHUKF_UI=1 RHUKF_R=1 \
    RHUKF_PD=0.2 RHUKF_ALPHA=0.10 \
...
echo "[$(date '+%m-%d %H:%M')] swirl_winner END rc=$rc ep=$ep" | tee -a isaac_winner.log
touch $OUT/RUN_DONE
for p in $(ps -eo pid,args|grep "isaacsim/kit"|grep -v grep|awk '{print $1}'); do kill -9 $p 2>/dev/null||true; done
