#!/usr/bin/env python3
"""env_scan_v9 — A: 관측 잡음 레짐 (CartPole 대응): obs_noise σ {0, 0.2, 0.4} × SWIRL top-3 (P0.05·R3·N7 / P0.1·R3·N7 / P0.1·R3·N6, Q1e-3·α0.1) + Adam × seed {42,43} = 24. 무대 R0 (plateau U(15,30)·상단 0.3).
...
def run(name, env, ag, sd, extra):
    ...
    t0 = time.time(); h = SR.run_config(env, n_ep=200, seed=sd, ep_steps=300, agent_type=ag, obs_noise=float(extra.get('OBS_NOISE', '0')))
    ...
    return m
jobs = []
SW = {'SW1': dict(BASE, RHUKF_PINIT='0.05', RHUKF_R='3', RHUKF_N='7'), 'SW2': dict(BASE, RHUKF_PINIT='0.1', RHUKF_R='3', RHUKF_N='7'), 'SW3': dict(BASE, RHUKF_PINIT='0.1', RHUKF_R='3', RHUKF_N='6')}
for nz in ('0.0', '0.2', '0.4'):
    for ag in ('SW1', 'SW2', 'SW3', 'Adam'):
        for sd in (42, 43):
            envv, agt = (SW[ag], 'rhukf') if ag != 'Adam' else (AG['Adam'][0], 'adam')
            jobs.append((f'nz{nz}_{ag}_s{sd}', envv, agt, sd, {'ATK_ON_LO': '15', 'ATK_ON_HI': '30', 'ATK_P_UPPER': '0.3', 'OBS_NOISE': nz}))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v9_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v9_w{WID}.json', 'w'), default=float)
open(f'{N}/V9_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
