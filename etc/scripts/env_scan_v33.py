#!/usr/bin/env python3
"""env_scan_v33 (09-15 오후, 사용자 질문) — ① 보상 패널티 크기 k(FP·FN 항 ×k, TP/TN 고정) 가 학습기 격차를 키우나
...
def pen(k): return dict(R_FP=f'{-0.7*k:.3f}', FN_BASE=f'{-0.4*k:.3f}', FN_PER=f'{-0.35*k:.3f}')
def markov(sd):
    rng = np.random.default_rng(1000 + sd); segs = []; ep = 0; calm = True
    while ep < 150:
        e = min(ep + int(rng.integers(20, 41)) - 1, 149)
        segs.append(f"{ep}-{'' if e >= 149 else e}:{'0:0.6,7:0.4' if calm else '10:1'}"); ep = e + 1; calm = not calm
    return ';'.join(segs)
CELLS = {'blk50k_k2': lambda sd: dict(SURR_TIER_SCHED=BLOCK, BUFFER_SIZE='50000', **pen(2)),
         'blk50k_k4': lambda sd: dict(SURR_TIER_SCHED=BLOCK, BUFFER_SIZE='50000', **pen(4)),
         'mix50k_k4': lambda sd: dict(SURR_TIER_P=MIX, BUFFER_SIZE='50000', **pen(4)),
         'mkv50k':    lambda sd: dict(SURR_TIER_SCHED=markov(sd), BUFFER_SIZE='50000')}
JOBSET = os.environ.get('JOBSET', 'gpu'); SEEDS = range(42, 47)
jobs = []
for cn, cf in CELLS.items():
    ...
    for sd in SEEDS:
        for ag, envv, agt in ags:
            jobs.append((f'{cn}_{ag}_s{sd}', envv, agt, sd, cf(sd)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v33{JOBSET}_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v33{JOBSET}_w{WID}.json', 'w'), default=float)
open(f'{N}/V33{JOBSET}_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
