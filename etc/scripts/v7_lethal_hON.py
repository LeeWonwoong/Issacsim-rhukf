...
SW1=dict(NET_HIDDEN=16,RHUKF_N=6,RHUKF_Q='1e-3',RHUKF_TAU='0.005',RHUKF_UI=1,RHUKF_FORM='absolute',RHUKF_SPAS=1,RHUKF_ALPHA='0.1',RHUKF_R='1',RHUKF_PINIT='0.03',RHUKF_HUBER_C='3.0')
SW2=dict(SW1,RHUKF_PINIT='0.01')
jobs=[]
for L in ('0.4','0.5'):
    jobs.append((f'SWIRL_P03_L{L}',dict(SW1),'rhukf',L))
    jobs.append((f'SWIRL_P01_L{L}',dict(SW2),'rhukf',L))
    jobs.append((f'Adam_L{L}',dict(NET_HIDDEN=16,ADAM_LR='3e-4'),'adam',L))
    jobs.append((f'SGD_L{L}',dict(NET_HIDDEN=16,ADAM_LR='3e-4',OPT='sgd'),'adam',L))
mine=jobs[WID::2]; out={}
print(f'[w{WID}] {len(mine)} runs',flush=True)
for n,e,ag,L in mine:
    os.environ['SURR_P_LETHAL']=L
    out[n]=go(n,e,ag)
json.dump(out,open(f'/tmp/v7lthON_w{WID}.json','w'),default=float); open(f'V7LTHON_W{WID}_DONE','w').close()
