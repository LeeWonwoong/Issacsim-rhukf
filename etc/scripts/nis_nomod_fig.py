#!/usr/bin/env python3
"""nis_nomod_fig — 속도변조 on(bandcap_arm005/results_bandcap) vs off(nomod) time-step NIS, G2+ 필터·클립 4.0, 6패널"""
...
def replay_seg(data, dt, fresh, s0, s1):
    ...
    return nv, ng
CACHE = {}
def series(D, delta, ws, pattern, T=np.arange(0, 300)):
...
MOD6 = 'results/claudecodefortest/bandcap_arm005'; MOD0 = 'results_bandcap'; NOM = 'results/claudecodefortest/nomod'
PANELS = [(0.0, 6, 'circle'), (0.0, 6, 'figure8'), (0.0, 0, 'scurve'), (0.25, 6, 'circle'), (0.25, 6, 'figure8'), (0.25, 0, 'scurve'), (0.0, 6, 'waypoint'), (0.0, 6, 'aggressive'), (0.8, 6, 'aggressive')]
T = np.arange(0, 300); fig, ax = plt.subplots(3, 3, figsize=(18, 9.5), sharex=True)
for a, (dl, ws, p) in zip(ax.ravel(), PANELS):
    print('panel', dl, ws, p, flush=True)
    for D, lab, ls, lw in ((MOD6 if ws == 6 else MOD0, '변조 on', ':', 1.4), (NOM, '변조 off', '-', 2.0)):
        g, v, n, cr = series(D, dl, ws, p)
        if g is None: continue
        a.plot(T, g, color='tab:red', ls=ls, lw=lw, label=f'gyro {lab} (n{n})'); a.plot(T, v, color='tab:blue', ls=ls, lw=lw, label=f'vel {lab}')
        for c in cr: a.axvline(c, color='k' if ls == '-' else 'gray', lw=0.8, ls=':')
    if ws > 0: a.axvspan(60, 290, color='c', alpha=.06)
    if dl > 0: a.axvspan(ATK0, ATK0 + 30, color='r', alpha=.12)
    a.set_ylim(0, 4.1); a.grid(alpha=.3); a.set_title(f'{p} · δ{dl} · ws{ws} (track)', fontsize=10); a.legend(fontsize=7, ncol=2, loc='upper left')
for a in ax[-1]: a.set_xlabel('step (10 Hz)')
fig.suptitle('속도변조 on(점선, amp 0.5·0.5 Hz) vs off(실선) — time-step NIS, G2+ 필터, 클립 4.0, gyro 빨강 / vel 파랑 · 빨간 띠 = 공격 ON 3 s')
fig.tight_layout(); fig.savefig('results/claudecodefortest/night/nis_nomod_compare.png', dpi=100); print('saved', flush=True)
