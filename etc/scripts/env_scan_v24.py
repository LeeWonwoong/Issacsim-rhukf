...
# v24 (09-13 밤): 하한 δ0.05 가족 × {SWIRL, Adam, UKF-TD(paper/matched), EKF-TD(paper/matched)} × seed{42,43}
COMMON = dict(STAGE, EPS_HOVER_P='0.1', EPS_DECAY='5000', GAMMA='0.9', HOVER_DWELL='5', SURR_V5='1', SURR_WEAK_INTERP='1')
CELLS = {'lo05':       dict(COMMON, ATK_DELTA_LO='0.05'),                                        # weak U(.05,.72) 50% + strong 50%
         'lo05w20p30': dict(COMMON, ATK_DELTA_LO='0.05', ATK_WEAK_HI='0.2', ATK_P_UPPER='0.3')}   # 스텔스 U(.05,.2) 70% + strong 30%
NOISE = {}
SWB  = dict(BASE, RHUKF_PINIT='0.03', RHUKF_R='1', RHUKF_N='6')                                                       # SWIRL(FIR 창 6, SPAS, Huber 3)
KTD  = dict(NET_HIDDEN=16, RHUKF_TAU='0.005', RHUKF_UI=1, RHUKF_FORM='absolute', RHUKF_SPAS=0, RHUKF_ALPHA='0.1', RHUKF_Q='1e-3', RHUKF_N='1')
AGENTS = {'SW':    (SWB, 'rhukf'),
          'Adam':  (dict(AG['Adam'][0], ADAM_AMSGRAD='0'), 'adam'),
          'UKFp':  (dict(KTD, RHUKF_MODE='ukf', RHUKF_PINIT='1.0',  RHUKF_R='1', RHUKF_HUBER_C='1e9'), 'rhukf'),   # KTD 논문형: P0 1, P_n 1, P_v 1e-3, Huber 없음
          'UKFm':  (dict(KTD, RHUKF_MODE='ukf', RHUKF_PINIT='0.03', RHUKF_R='1', RHUKF_HUBER_C='3'), 'rhukf'),     # SWIRL 과 동일 P0·R·Huber, FIR/SPAS 만 제거
          'EKFp':  (dict(KTD, RHUKF_MODE='ekf', RHUKF_PINIT='1.0',  RHUKF_R='1', RHUKF_HUBER_C='1e9'), 'rhukf'),   # KOVA/Singhal–Wu 형
          'EKFm':  (dict(KTD, RHUKF_MODE='ekf', RHUKF_PINIT='0.03', RHUKF_R='1', RHUKF_HUBER_C='3'), 'rhukf')}
for cn, cv in CELLS.items():
    for ag, (envv, agt) in AGENTS.items():
        for sd in (42, 43):
            jobs.append((f'{cn}_{ag}_s{sd}', envv, agt, sd, cv))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v24_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v24_w{WID}.json', 'w'), default=float)
open(f'{N}/V24_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
