...
kill_all; OUT=results_trajcheck_ff; rm -rf $OUT; mkdir -p $OUT
echo "[$(date '+%m-%d %H:%M')] trajcheck START" | tee isaac_trajcheck_ff.log
...
timeout 3600 python3 -u online_rl_main.py --headless --sweep --sweep-mode torque --capture-mode hijack \
  ...
  --capture-biases 0 --episodes 1 --speed 2.5 --outdir $OUT > $OUT/run.log 2>&1
echo "[$(date '+%m-%d %H:%M')] trajcheck END rows=$(wc -l < $OUT/sweep_summary.csv 2>/dev/null||echo 0)" | tee -a isaac_trajcheck_ff.log
kill_all; touch TRAJCHECK_FF_DONE
