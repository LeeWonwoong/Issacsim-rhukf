#!/usr/bin/env bash
# slowcap (2026-09-11) — 궤적 속도 −15% (TRAJ_SCALE=0.70, PX4 캡 불변) 에서 밴드·aliasing·추종 재측정. arm 0.05.
...
while [ ! -f results/claudecodefortest/SLOWCAP_DONE ]; do sleep 60; done
set +u; source /opt/ros/humble/setup.bash 2>/dev/null||true; source ~/colcon_ws/install/setup.bash 2>/dev/null||true
source etc/frozen_v3.env; set -u; unset MC_INT_LIM MPC_TILTMAX MPC_ACC_HOR HOVER_HARD_GAIN
export SPEED_SCALE=1.0 TRAJ_SCALE=0.70 WIND_MOMENT_ARM=0.05 EP_MAX_STEPS=300 SWEEP_ATTACK_TYPE=tilt
kill_all(){ pkill -f MicroXRCEAgent 2>/dev/null||true
  for p in $(pgrep -f "python.sh run_sim" 2>/dev/null); do kill -9 $p 2>/dev/null||true; done
  for p in $(ps -eo pid,args|grep "isaacsim/kit"|grep -v grep|awk '{print $1}'); do kill -9 $p 2>/dev/null||true; done; sleep 6; }
kill_all; OUT=results/claudecodefortest/slowcap30; rm -rf $OUT; mkdir -p $OUT
echo "[$(date '+%m-%d %H:%M')] slowcap START (TRAJ_SCALE 0.85, arm 0.05)" | tee results/claudecodefortest/isaac_slowcap30.log
...
echo "[$(date '+%m-%d %H:%M')] slowcap END rows=$(wc -l < $OUT/sweep_summary.csv 2>/dev/null||echo 0)" | tee -a results/claudecodefortest/isaac_slowcap30.log
(setsid nohup python3 etc/scripts/analyze_bandcap.py $OUT 30 > $OUT/analysis.log 2>&1 < /dev/null &)
kill_all; touch results/claudecodefortest/SLOWCAP30_DONE
