...
CELLS = {'gU': dict(GUST, REPLAY_MODE='uniform', BUFFER_SIZE='50000'), 'gC': dict(GUST, REPLAY_MODE='cer', BUFFER_SIZE='50000'),
         'gR': dict(GUST, REPLAY_MODE='recency', REPLAY_HALFLIFE='3000', BUFFER_SIZE='50000'), 'gS': dict(GUST, REPLAY_MODE='uniform', BUFFER_SIZE='5000'),
         'nU': dict(FRAME, REPLAY_MODE='uniform', BUFFER_SIZE='50000')}
NOISE = {}
...
for cn, cv in CELLS.items():
    if JOBSET == 'gpu':
        ags = [('SW', SWB, 'rhukf')] if cn == 'nU' else ([] if cn == 'gS' else [('SW', SWB, 'rhukf'), ('UKFm', dict(KTD, RHUKF_MODE='ukf'), 'rhukf')])   # 시간 제약: gS·EKF GPU 셀 제외
    else:
        ags = [('Adam', ADAMP, 'adam')]
...
try: out = json.load(open(f'{N}/v27{JOBSET}_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v27{JOBSET}_w{WID}.json', 'w'), default=float)
open(f'{N}/V27{JOBSET}_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
