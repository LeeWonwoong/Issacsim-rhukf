...
CELLS = {'chainR30': dict(CH, SURR_WEAK_RISE_MAX='30', SURR_P_LETHAL='0.3'),
         'chainR0':  dict(CH, SURR_WEAK_RISE_MAX='0',  SURR_P_LETHAL='0.3'),
         'chainR30L10': dict(CH, SURR_WEAK_RISE_MAX='30', SURR_P_LETHAL='0.1'),
         'single_ref': dict(STAGE, EPS_HOVER_P='0.1', GAMMA='0.9', EPS_DECAY='4000', HOVER_DWELL='5')}
NOISE = {}
for cn, cv in CELLS.items():
    for sd in (42, 43, 44):
        jobs.append((f'{cn}_AdamNoAms_s{sd}', dict(AG['Adam'][0], ADAM_AMSGRAD='0'), 'adam', sd, cv))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v22_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v22_w{WID}.json', 'w'), default=float)
open(f'{N}/V22_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
