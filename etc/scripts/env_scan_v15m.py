...
for D in (5, 10):
    for ag in ('SW1', 'SW4'):
        jobs.append((f'R0d{D}e8000g0.95_{ag}_s42', SW[ag], 'rhukf', 42, dict(STAGE, **COMMON, GAMMA='0.95', EPS_DECAY='8000', HOVER_DWELL=str(D))))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v15m_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v15m_w{WID}.json', 'w'), default=float)
open(f'{N}/V15m_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
