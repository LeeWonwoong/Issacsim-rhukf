#!/usr/bin/env python3
# v51_transfer (2026-09-04) — env5(Isaac 학습분포 풀, 인공노이즈 0)가 Isaac 랭킹을 재현하는가.
...
os.environ['SURR_V51'] = '1'; os.environ.pop('SURR_V4', None)
os.environ['SURR_ATK_PROB'] = '0.70'
OUT = 'results_v51'; os.makedirs(OUT, exist_ok=True)
...
W0 = [(f'v51_P{p}', 'rhukf', dict(B, RHUKF_PINIT=p), ISAAC[p]) for p in ('0.002','0.01','0.05')]
W1 = [(f'v51_P{p}', 'rhukf', dict(B, RHUKF_PINIT=p), ISAAC[p]) for p in ('0.005','0.02','0.1')]
W1 += [('v51_adam3e4','adam',dict(NET_HIDDEN=16,ADAM_LOSS='mse',ADAM_AMSGRAD=0,ADAM_LR='3e-4'),0.927),
       ('v51_adam1e3','adam',dict(NET_HIDDEN=16,ADAM_LOSS='mse',ADAM_AMSGRAD=0,ADAM_LR='1e-3'),0.924)]
...
for name, ag, env, isaac_f1 in mine:
    ...
    json.dump(out, open(f'{OUT}/v51_w{WID}.json','w'), default=float)
open(f'{OUT}/V51_W{WID}_DONE','w').close(); print(f'[worker {WID}] done', flush=True)
