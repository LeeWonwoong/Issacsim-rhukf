#!/usr/bin/env python3
"""env_scan_v17 (09-13) — 비정상성 축: ep100 부터 관측 노이즈 0.4 / NIS 배율 0.7 × {SW1,SW4,Adam} × seed{42,43}, D5·γ0.95·decay8000, 200ep. 회복 속도(ep100–130 F1) 비교. 원본 v15 — 약속 hover D∈{5,10} × γ{0.85,0.9,0.95} × ε decay {4000,8000,12000} × 4항 보상 × {SW1,SW4,Adam} × seed{42,43} = 36 런 + Kurt형 ablation(D10·8000·s42) 3 런. hover 탐험 0.05, γ0.95, 200ep, v5b 풀 (사용자 09-13 03:30: D≤10, decay 3종). 원본 v14 — A3 약속 hover(최소 체류 D∈{10,20} 강제, 보상 track +p−c·1{atk}, hover 0, FA=잃은 진행) × {SW1,SW4,Adam} × seed{42,43} + D0 대조, 200ep, v5b 풀, P(atk) 0.5. 원본 v13 —: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
BASE17 = dict(STAGE, EPS_HOVER_P='0.05', GAMMA='0.95', EPS_DECAY='8000', HOVER_DWELL='5', SURR_SHIFT_EP='100')
for sn, sv in (('noise0.4', dict(SURR_SHIFT_NOISE='0.4')), ('scale0.7', dict(SURR_SHIFT_SCALE='0.7'))):
    jobs.append((f'shift{sn}_SW1_s42', SW['SW1'], 'rhukf', 42, dict(BASE17, **sv)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v17m_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v17m_w{WID}.json', 'w'), default=float)
open(f'{N}/V17m_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
