...
def main():
    ...
    ap.add_argument('--hover-altmin', type=float, default=1.2, help='정지 판정 최소고도 [m]')
    ap.add_argument('--cruise-vmin', type=float, default=0.8, help='등속 판정 최소속도 [m/s]')
    args = ap.parse_args()
    ...
    if C_thrust is None:
        ...
        segs2 = runs(fast, t_lp, 1.2)
        if not segs2:
            print(f"  등속 구간 없음 (|vh|>{args.cruise_vmin} m/s, 1.2초 이상)")
