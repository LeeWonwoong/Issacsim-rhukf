...
def main():
    ...
    if args.type == 'hover':
        ...
        ALTMIN = 1.2      # 저고도(2.0~2.5m) 운용 기준. 지면효과는 0.5m 이하에서만 유의
        good = (vh < 0.3) & (alt > ALTMIN) if alt is not None else (vh < 0.3)
        seg = longest_run(good, t_lp)
        print(f"       정지 구간(|vh|<0.3 m/s, 고도>{ALTMIN}m) 최장 = {seg:.1f}s")
        if seg < 45:
        ...
        VMIN = 0.8      # 저속 설정(v=1.0~1.5)에서도 등속 구간을 잡도록
        fast = vh > VMIN
        seg = longest_run(fast, t_lp)
        print(f"       |vh|>{VMIN} m/s 최장 연속 = {seg:.1f}s   (등속 구간 1.5초 이상 필요)")
        if seg < 1.5:
