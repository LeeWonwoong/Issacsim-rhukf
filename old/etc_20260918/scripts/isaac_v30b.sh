#!/usr/bin/env bash
# isaac_v30 (2026-09-15) — surrogate v30 급변 블록 무대를 Isaac 에서 검증: 시드 1(42), 150 ep, {swirl P0.03·R1·N7, adam 3e-4, ukf-TD, ekf-TD} 순차.
...
run_one(){ NAME=$1; AGENT=$2; shift 2; OUT=results/claudecodefortest/isaac_v30_$NAME
...
# swirl 은 GPU 경합(learn-step 62ms > 예산 40ms) 때문에 마지막: surrogate v31 완료(V31gpu DONE 8개) 후 실행
run_one adam  adam  $ADAM
run_one ukf   rhukf $KTD RHUKF_MODE=ukf
run_one ekf   rhukf $KTD RHUKF_MODE=ekf
N=results/claudecodefortest/night; while [ "$(ls $N | grep -c "V31gpu_w.*_DONE")" -lt 8 ]; do sleep 300; done; log "v31 완료 → isaac_v30_swirl 착수(GPU 단독)"
run_one swirl rhukf $SW
kill_all; touch results/claudecodefortest/ISAAC_V30_DONE; log "isaac_v30 전체 완료"
