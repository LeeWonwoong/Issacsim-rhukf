...
def run(name, env, ag, sd, extra):
    ...
    return m
jobs = []
SW = {'SW4': dict(BASE, RHUKF_PINIT='0.03', RHUKF_R='1', RHUKF_N='6')}
...
try: out = json.load(open(f'{N}/v9b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v9b_w{WID}.json', 'w'), default=float)
open(f'{N}/V9b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
