#!/usr/bin/env python3
"""env_scan_v20 (09-13 오후) — 스텔스 대역(δ<0.1 보간) 가족 × Adam 빠른 확인 + 관측노이즈 1.0/1.5 대조. 원본 v15 — 약속 hover D∈{5,10} × γ{0.85,0.9,0.95} × ε decay {4000,8000,12000} × 4항 보상 × {SW1,SW4,Adam} × seed{42,43} = 36 런 + Kurt형 ablation(D10·8000·s42) 3 런. hover 탐험 0.05, γ0.95, 200ep, v5b 풀 (사용자 09-13 03:30: D≤10, decay 3종). 원본 v14 — A3 약속 hover(최소 체류 D∈{10,20} 강제, 보상 track +p−c·1{atk}, hover 0, FA=잃은 진행) × {SW1,SW4,Adam} × seed{42,43} + D0 대조, 200ep, v5b 풀, P(atk) 0.5. 원본 v13 —: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
BASE20 = dict(EPS_HOVER_P='0.05', GAMMA='0.85', EPS_DECAY='4000', HOVER_DWELL='5', ATK_ON_LO='25', ATK_ON_HI='40', SURR_WEAK_INTERP='1')
CELLS = {
 'st03w20p30': dict(BASE20, ATK_DELTA_LO='0.03', ATK_WEAK_HI='0.2', ATK_P_UPPER='0.3'),   # 스텔스 U(.03,.2) 70% : 강 U(.72,.84) 30%
 'st03w20p50': dict(BASE20, ATK_DELTA_LO='0.03', ATK_WEAK_HI='0.2', ATK_P_UPPER='0.5'),
 'st05w30p30': dict(BASE20, ATK_DELTA_LO='0.05', ATK_WEAK_HI='0.3', ATK_P_UPPER='0.3'),
 'st03w20only': dict(BASE20, ATK_DELTA_LO='0.03', ATK_WEAK_HI='0.2', ATK_P_UPPER='0.0'),  # 약공격만(절벽 없음)
 'st03w10p30': dict(BASE20, ATK_DELTA_LO='0.03', ATK_WEAK_HI='0.1', ATK_P_UPPER='0.3'),   # 더 애매
 'ref_v5':     dict(BASE20, ATK_P_UPPER='0.5'),                                            # 현행 가족 대조
 'noise1.0':   dict(BASE20, ATK_P_UPPER='0.5'),
 'noise1.5':   dict(BASE20, ATK_P_UPPER='0.5'),
}
NOISE = {'noise1.0': 1.0, 'noise1.5': 1.5}
for cn, cv in CELLS.items():
    for sd in (42, 43):
        jobs.append((f'{cn}_Adam_s{sd}', AG['Adam'][0], 'adam', sd, cv))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v20_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v20_w{WID}.json', 'w'), default=float)
open(f'{N}/V20_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
