#!/usr/bin/env python3
"""env_scan_v10 — A2 정산 보상 vs R0 (스케일 없음): R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
def run(name, env, ag, sd, extra):
    ...
    t0 = time.time(); h = SR.run_config(env, n_ep=250, seed=sd, ep_steps=300, agent_type=ag)
...
A2 = dict(STAGE, REWARD_MODE='A2', A2_P='0.5', A2_C='6.0', A2_C0='0.5', A2_F='1.0', A2_INSTALL='5', GAMMA='0.95', EPS_DECAY='12000')
CELLS = {'R0': dict(STAGE), 'A2': A2}
for cn, cv in CELLS.items():
    for ag in ('SW1', 'SW2', 'SW4', 'Adam'):
        for sd in (42, 43):
            envv, agt = (SW[ag], 'rhukf') if ag != 'Adam' else (AG['Adam'][0], 'adam')
            jobs.append((f'{cn}_{ag}_s{sd}', envv, agt, sd, cv))
for cn, cv in (('A2cp0.7', dict(A2, A2_C='0.35')), ('A2noinst', dict(A2, A2_INSTALL='1'))):
    for ag in ('SW1', 'Adam'):
        envv, agt = (SW[ag], 'rhukf') if ag != 'Adam' else (AG['Adam'][0], 'adam')
        jobs.append((f'{cn}_{ag}_s42', envv, agt, 42, cv))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v10_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v10_w{WID}.json', 'w'), default=float)
open(f'{N}/V10_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
