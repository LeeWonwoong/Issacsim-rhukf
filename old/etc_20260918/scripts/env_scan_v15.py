#!/usr/bin/env python3
"""env_scan_v15 (09-13) — 약속 hover D∈{5,10} × γ{0.85,0.9,0.95} × ε decay {4000,8000,12000} × 4항 보상 × {SW1,SW4,Adam} × seed{42,43} = 36 런 + Kurt형 ablation(D10·8000·s42) 3 런. hover 탐험 0.05, γ0.95, 200ep, v5b 풀 (사용자 09-13 03:30: D≤10, decay 3종). 원본 v14 — A3 약속 hover(최소 체류 D∈{10,20} 강제, 보상 track +p−c·1{atk}, hover 0, FA=잃은 진행) × {SW1,SW4,Adam} × seed{42,43} + D0 대조, 200ep, v5b 풀, P(atk) 0.5. 원본 v13 —: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
A3 = dict(STAGE, REWARD_MODE='A3', A3_P='0.5', A3_C='2.0')
COMMON = dict(EPS_HOVER_P='0.05')
for D in (5, 10):
    for g in ('0.85', '0.9', '0.95'):
        for dec in ('4000', '8000', '12000'):
            for ag in ('SW1', 'SW4', 'Adam'):
                for sd in ((42, 43) if g == '0.95' else (42,)):   # 시간 제약(11:30): γ0.95 만 2시드, 0.85/0.9 는 1시드
                    envv, agt = (SW[ag], 'rhukf') if ag in SW else (AG['Adam'][0], 'adam')
                    jobs.append((f'R0d{D}e{dec}g{g}_{ag}_s{sd}', envv, agt, sd, dict(STAGE, **COMMON, GAMMA=g, EPS_DECAY=dec, HOVER_DWELL=str(D))))
for ag in ('SW1', 'SW4', 'Adam'):   # Kurt 형 보상 ablation (D10, γ0.95, decay 8000, seed 42)
    envv, agt = (SW[ag], 'rhukf') if ag in SW else (AG['Adam'][0], 'adam')
    jobs.append((f'A3d10e8000g0.95_{ag}_s42', envv, agt, 42, dict(A3, **COMMON, GAMMA='0.95', EPS_DECAY='8000', HOVER_DWELL='10')))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v15_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v15_w{WID}.json', 'w'), default=float)
open(f'{N}/V15_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
