...
CELLS = {f'g{g}': dict(STAGE, EPS_HOVER_P='0.1', GAMMA=g, EPS_DECAY='4000', HOVER_DWELL='5') for g in ('0.85', '0.9', '0.95')}
NOISE = {}
for cn, cv in CELLS.items():
    for ag in ('SW',):
        for sd in (42, 43, 44, 45, 46):
            envv, agt = (SWB, 'rhukf') if ag == 'SW' else (AG['Adam'][0], 'adam')
            jobs.append((f'{cn}_SW_s{sd}', envv, agt, sd, cv))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v21b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v21b_w{WID}.json', 'w'), default=float)
open(f'{N}/V21b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
