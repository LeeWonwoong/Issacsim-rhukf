...
for f in glob.glob(f'{N}/tonight/q/final4*_s42.json'):
...
LS = ['SW', 'UKFp03', 'EKFp03', 'Adam3e-4', 'Adam1e-3', 'SGD1e-2']
REF = 'SW'   # final4 기준 SWIRL(P0.03·N7)
...
def strong(c, l, key, a, b):   # 절벽 구간(δ≥0.74) 판만
    ...
    return float(np.mean(v)) if v else np.nan
for cell, lab in (('xfinal4', 'final4 클립제거 [0,4]'), ('xfinal4n', 'final4n 클립제거+/4 [0,1]')):
