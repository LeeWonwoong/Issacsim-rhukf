#!/usr/bin/env python3
"""env_scan_v30 (09-14 저녁, 사용자 스펙) — 급변 비정상성 축 = 강풍 블록: 학습 중간 구간(ep 60–109)만 ws10(실측 aliasing ×2.28) 100 %,
...
if WID == 0: open(f'{N}/V30_SWIRL_CFG', 'w').write(f'P={P} R={R} N={Nh} ({src})\n')
print(f'[v30] SWIRL cfg P={P} R={R} N={Nh} ({src})', flush=True)
BLOCK = '0-59:0:0.6,7:0.4;60-109:10:1;110-:0:0.6,7:0.4'
CELLS = {'blk50k': dict(SURR_TIER_SCHED=BLOCK, BUFFER_SIZE='50000'), 'blk10k': dict(SURR_TIER_SCHED=BLOCK, BUFFER_SIZE='10000'),
         'mix50k': dict(SURR_TIER_P='0:0.4,7:0.3,10:0.3', BUFFER_SIZE='50000'), 'mix10k': dict(SURR_TIER_P='0:0.4,7:0.3,10:0.3', BUFFER_SIZE='10000')}
...
try: out = json.load(open(f'{N}/v30{JOBSET}_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v30{JOBSET}_w{WID}.json', 'w'), default=float)
open(f'{N}/V30{JOBSET}_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
