...
"""nis_mid_fig — 중간 필터 M(Q_gyro 1e-2·R_gyro 0.1·Q_euler 5e-4·R_vel 0.1) 을 E·G2 와 겹친 대표 time-step NIS 그림 6패널.
   ...
   출력: results/claudecodefortest/night/nis_g2plus.png"""
...
A = 4.36; ATK0 = 180
CFGS = {'E': dict(), 'M': dict(qg=2e-3, rg=0.2, qe=2e-3, qv=1e-3, rv=0.1), 'G2': dict(qg=2e-3, rg=0.2, qe=5e-4, rv=0.1)}
NM = {'E':'E','M':'G2+','G2':'G2'}
...
def replay_seg(data, dt, fresh, s0, s1, cfg):
    ...
    for sl, v in ((slice(3, 6), cfg.get('qe')), (slice(6, 9), cfg.get('qv')), (slice(9, 12), cfg.get('qg'))):
        if v is not None:
...
for a, (D, dl, ws, p, on, title) in zip(ax.ravel(), PANELS):
    ...
    for nm in ('E', 'G2', 'M'):
        ...
        a.plot(T, g, color='tab:red', label=f'gyro {NM[nm]}', **st); a.plot(T, v, color='tab:blue', label=f'vel {NM[nm]}', **st)
    ...
    a.set_ylim(0, 4.1); a.grid(alpha=.3); a.set_title(f'{title} (track, n={n})', fontsize=10)
    if a is ax[0][0]: a.legend(fontsize=7, ncol=4, loc='upper left')
for a in ax[-1]: a.set_xlabel('step (10 Hz)')
fig.suptitle('time-step NIS — E(가는 실선) · G2(점선) · G2+ = Qg2e-3·Rg0.2·Qe2e-3·Qv1e-3·Rv0.1 (굵은 실선) · 클립 4.0 · gyro 빨강 / vel 파랑 · 빨간 띠=공격 ON 3 s · 세로 점선=추락')
fig.tight_layout(); os.makedirs('results/claudecodefortest/night', exist_ok=True)
fig.savefig('results/claudecodefortest/night/nis_g2plus.png', dpi=110); print('saved', flush=True)
