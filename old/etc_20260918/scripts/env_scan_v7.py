#!/usr/bin/env python3
"""env_scan_v7 — 보상구조 스캔: R0 현행 / R1 희소화(TP·FP·FN ×0.2, TN +0.5 유지 = 임무 진행 보상 우세) / R2 γ0.9 / R12 / R123(+ε decay 12000)
...
def run(name, env, ag, sd, extra):
    ...
    return m
jobs = []
R = {'R0': {}, 'R1': {'R_TP': '0.2', 'R_FP': '-0.14', 'FN_BASE': '-0.08', 'FN_PER': '-0.07'}, 'R2': {'GAMMA': '0.9'},
     'R12': {'R_TP': '0.2', 'R_FP': '-0.14', 'FN_BASE': '-0.08', 'FN_PER': '-0.07', 'GAMMA': '0.9'},
     'R123': {'R_TP': '0.2', 'R_FP': '-0.14', 'FN_BASE': '-0.08', 'FN_PER': '-0.07', 'GAMMA': '0.9', 'EPS_DECAY': '12000'}}
for rn, rv in R.items():
    for ag in ('SW1', 'Adam'):
        for sd in (42, 43):
            jobs.append((f'{rn}_{ag}_s{sd}', AG[ag][0], AG[ag][1], sd, dict({'ATK_ON_LO': '15', 'ATK_ON_HI': '30', 'ATK_P_UPPER': '0.3'}, **rv)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v7_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v7_w{WID}.json', 'w'), default=float)
open(f'{N}/V7_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
