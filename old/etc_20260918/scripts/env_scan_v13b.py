#!/usr/bin/env python3
"""env_scan_v13 (09-13) — 희소성 축: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
SGD = dict(NET_HIDDEN=16, ADAM_LR='3e-4', OPT='sgd')
for pa in ('0.3',):
    for ag in ('SW1', 'SW4', 'Adam', 'SGD'):
        for sd in (42, 43, 44):
            envv, agt = (SW[ag], 'rhukf') if ag in SW else ((AG['Adam'][0], 'adam') if ag == 'Adam' else (SGD, 'adam'))
            jobs.append((f'pa{pa}_{ag}_s{sd}', envv, agt, sd, dict(STAGE, SURR_ATK_PROB=pa)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v13b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v13b_w{WID}.json', 'w'), default=float)
open(f'{N}/V13b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
