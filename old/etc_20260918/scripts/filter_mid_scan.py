#!/usr/bin/env python3
"""filter_mid_scan — E 와 G2 사이 중간 필터 탐색 (오프라인 zu 재생, 병렬).
   축: Q_gyro {2e-3, 5e-3, 1e-2} × R_gyro {0.05, 0.1, 0.2}, Q_euler 5e-4 · R_vel 0.1 (G2 의 vel 설정) + 기준 E, G2.
   데이터: bandcap_arm005 (ws6: clean/바람/δ0.25/δ0.74~0.82) + weakfloor (δ0.1~0.2, ws0/ws6).
   지표: 바람 gyro p95/p99 · δ0.25/0.15 gyro min · 진폭여유(바람p95/공격min) · 클립비율 · vel AUC(바람·공격) · OFF후 vel"""
...
CFGS = {'E': dict(), 'G2': dict(qg=2e-3, rg=0.2, qe=5e-4, rv=0.1)}
for qg in ('2e-3', '5e-3', '1e-2'):
    for rg in ('0.05', '0.1', '0.2'):
        if (qg, rg) == ('2e-3', '0.2'): continue
...
if __name__ == '__main__':
    ...
    json.dump(res, open(f'{R}/night/filter_mid_scan.json', 'w'), default=float)
    print(f"{'config':14s} {'wind med/p95/p99':>18s} {'δ.25 min/med':>13s} {'clip.25/.74':>11s} {'여유 p95/min':>11s} {'δ.15 min':>8s} {'vel AUC wind/.25/.15':>20s} {'post.25 v/g':>11s}")
    for o in res:
        print(f"{o['name']:14s} {o['wind_med']:.2f}/{o['wind_p95']:.2f}/{o['wind_p99']:.2f}    {o['g0.25_min']:.2f}/{o['g0.25_med']:.2f}     {o['clip0.25']:.2f}/{o['clip0.74']:.2f}    {o['wind_p95']/o['g0.25_min']:.2f}      {o.get('wg0.15_min', float('nan')):.2f}    {o['wind_vel_auc']:.2f}/{o['v0.25_auc']:.2f}/{o.get('wv0.15_auc', float('nan')):.2f}      {o['post0.25_v']:.2f}/{o['post0.25_g']:.2f}")
