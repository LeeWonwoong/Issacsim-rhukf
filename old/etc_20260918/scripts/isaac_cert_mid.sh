#!/usr/bin/env bash
# cert_MID (2026-09-14) — 중간 세기 δ{0.4,0.5,0.6}(1.744/2.18/2.616 N·m) × ws{0,7,10} × 5패턴 × track × 4ep = 180 ep (~65 min): 패턴별 흐름(드리프트)·회복·NIS·aliasing, 상한 0.5 검토용
#   (cert_WIND δ0.5 는 패턴당 2ep 뿐이라 보강)
...
OUT=results/claudecodefortest/cert_MID; rm -rf $OUT; mkdir -p $OUT; kill_all; log "cert_MID START (δ0.4/0.5/0.6 × ws0/7/10 × track × 4ep)"
...
timeout 10800 python3 -u online_rl_main.py --headless --sweep --sweep-mode torque --capture-mode hijack --log-zu \
  ...
  --capture-biases 1.744,2.180,2.616 --episodes 4 --speed 2.5 --outdir $OUT > $OUT/run.log 2>&1
log "cert_MID END rows=$(wc -l < $OUT/sweep_summary.csv 2>/dev/null||echo 0) err=$(grep -c -E 'Traceback|HARD_RESET' $OUT/run.log)"
kill_all
