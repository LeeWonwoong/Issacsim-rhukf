#!/usr/bin/env python3
"""env_scan_v16 (09-13) — 프레임워크 성능 평가: v15 최적 셀에서 300ep × 3시드 × 3옵티마이저 (+ CUSUM 베이스라인은 surr_cusum_eval.py). 원본 v15 — 약속 hover D∈{5,10} × γ{0.85,0.9,0.95} × ε decay {4000,8000,12000} × 4항 보상 × {SW1,SW4,Adam} × seed{42,43} = 36 런 + Kurt형 ablation(D10·8000·s42) 3 런. hover 탐험 0.05, γ0.95, 200ep, v5b 풀 (사용자 09-13 03:30: D≤10, decay 3종). 원본 v14 — A3 약속 hover(최소 체류 D∈{10,20} 강제, 보상 track +p−c·1{atk}, hover 0, FA=잃은 진행) × {SW1,SW4,Adam} × seed{42,43} + D0 대조, 200ep, v5b 풀, P(atk) 0.5. 원본 v13 —: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
CLR = ('SURR_SHIFT_EP', 'SURR_SHIFT_NOISE', 'SURR_SHIFT_SCALE', 'REWARD_MODE', 'A2_P', 'A2_C', 'A2_C0', 'A2_F', 'A2_INSTALL', 'R_TP', 'R_FP', 'FN_BASE', 'FN_PER', 'RHUKF_PD', 'RHUKF_PINIT', 'RHUKF_FORM', 'RHUKF_ALPHA', 'RHUKF_R', 'RHUKF_N', 'RHUKF_HUBER_C', 'RHUKF_SPAS', 'RHUKF_Q', 'RHUKF_TAU', 'RHUKF_UI', 'ADAM_LR', 'OPT', 'ADAM_LOSS', 'RELAPSE_PEN', 'NSTEP_OFF')
...
# v15 결과에서 SW1 평균 F1 최고 셀(R0d{D}e{dec}g{g}) 선택 → 300ep × seed{42,43,44} × {SW1,SW4,Adam}
import glob, re
_v15 = {}
for f in glob.glob(f'{N}/v15_w*.json'): _v15.update(json.load(open(f)))
_cells = {}
for k, m in _v15.items():
    if not k.startswith('R0'): continue
    cell, ag = k.split('_')[0], k.split('_')[1]
    _cells.setdefault(cell, {}).setdefault(ag, []).append(float(m['F1']))
def _score(c):
    d = _cells[c]
    return float(np.mean(d['Adam'])) if 'Adam' in d else -1.0
best = max(_cells, key=_score) if _cells else 'R0d5e8000g0.95'
mm = re.match(r'R0d(\d+)e(\d+)g([0-9.]+)', best); D, dec, g = mm.group(1), mm.group(2), mm.group(3)
open('results/claudecodefortest/COMMIT_DECAY', 'w').write(dec + '\n'); open('results/claudecodefortest/COMMIT_GAMMA', 'w').write(g + '\n')
open('results/claudecodefortest/COMMIT_CFG', 'w').write(f'{best} D={D} decay={dec} gamma={g} AdamMeanF1={_score(best):.3f}\n')
print(f'[v16] best cell {best}: D={D} decay={dec} gamma={g} (Adam mean F1 {_score(best):.3f})', flush=True)
for ag in ('SW1', 'Adam'):
    for sd in (42, 43, 44):
        envv, agt = (SW[ag], 'rhukf') if ag in SW else (AG['Adam'][0], 'adam')
        jobs.append((f'{best}_{ag}_s{sd}', envv, agt, sd, dict(STAGE, EPS_HOVER_P='0.05', GAMMA=g, EPS_DECAY=dec, HOVER_DWELL=D)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v16_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v16_w{WID}.json', 'w'), default=float)
open(f'{N}/V16_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
