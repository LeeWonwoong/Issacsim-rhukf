...
CELLS = {'chainR30': dict(CH, SURR_WEAK_RISE_MAX='30', SURR_P_LETHAL='0.3'),
         ...
         'single':   dict(STAGE, EPS_HOVER_P='0.1', GAMMA='0.9', EPS_DECAY='4000', HOVER_DWELL='5', SURR_V5='1')}
NOISE = {}
import glob as _g, re as _re
_t = {}
for _f in _g.glob(f'{N}/v18_w*.json'): _t.update(json.load(open(_f)))
...
try: out = json.load(open(f'{N}/v22b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v22b_w{WID}.json', 'w'), default=float)
open(f'{N}/V22b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
