...
for sn, sv in (('noise0.4', dict(SURR_SHIFT_NOISE='0.4')), ('scale0.7', dict(SURR_SHIFT_SCALE='0.7'))):
    for ag in ('SW1', 'SW4', 'Adam'):
        for sd in (42, 43):
            ...
            jobs.append((f'shift{sn}_{ag}_s{sd}', envv, agt, sd, dict(BASE17, **sv)))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v17_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v17_w{WID}.json', 'w'), default=float)
open(f'{N}/V17_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
