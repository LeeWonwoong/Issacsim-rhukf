...
try: out = json.load(open(f'{N}/v6b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v6b_w{WID}.json', 'w'), default=float)
open(f'{N}/V6b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
