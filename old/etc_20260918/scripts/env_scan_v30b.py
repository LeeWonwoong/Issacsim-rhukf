...
if WID == 0: open(f'{N}/V30b_SWIRL_CFG', 'w').write(f'P={P} R={R} N={Nh} ({src})\n')
print(f'[v30b] SWIRL cfg P={P} R={R} N={Nh} ({src})', flush=True)
BLOCK = '0-59:0:0.6,7:0.4;60-109:10:1;110-:0:0.6,7:0.4'
CELLS = {'blk50k': dict(SURR_TIER_SCHED=BLOCK, BUFFER_SIZE='50000')}
JOBSET = os.environ.get('JOBSET', 'gpu'); SEEDS = range(47, 52)   # v30b: blk50k 시드 보강(47–51) → 합계 10
jobs = []
...
try: out = json.load(open(f'{N}/v30b{JOBSET}_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v30b{JOBSET}_w{WID}.json', 'w'), default=float)
open(f'{N}/V30b{JOBSET}_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
