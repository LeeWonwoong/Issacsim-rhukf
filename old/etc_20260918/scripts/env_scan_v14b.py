#!/usr/bin/env python3
"""env_scan_v14b (09-13) — 기존 4항 보상(R0) + 약속 hover 최소 체류 D∈{10,20}, γ0.95·decay12000, × {SW1,SW4,Adam} × seed{42,43}, 200ep. (A3 보상형과 비교) 원본 v14 — A3 약속 hover(최소 체류 D∈{10,20} 강제, 보상 track +p−c·1{atk}, hover 0, FA=잃은 진행) × {SW1,SW4,Adam} × seed{42,43} + D0 대조, 200ep, v5b 풀, P(atk) 0.5. 원본 v13 —: R0 보상 그대로, P(공격 에피) {0.1, 0.2} × {SW1, SW4, Adam, SGD} × seed {42,43,44}, 200ep, v5b 풀. 지표 = 학습곡선(hist) 로 F1 0.8 도달 에피·auc. 원본 v10:: R0(γ0.85·decay4000) / A2(γ0.95·decay12000·p0.5·c6·c0 0.5·f1·5분할) × {SW1,SW2,SW4,Adam} × seed{42,43}, 250 ep
...
for D in (10, 20):
    for ag in ('SW1', 'SW4', 'Adam'):
        for sd in (42, 43):
            ...
            jobs.append((f'R0d{D}_{ag}_s{sd}', envv, agt, sd, dict(STAGE, HOVER_DWELL=str(D), GAMMA='0.95', EPS_DECAY='12000')))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v14b_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v14b_w{WID}.json', 'w'), default=float)
open(f'{N}/V14b_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
