...
import glob as _g
_done = {}
for _f in _g.glob(f'{N}/v15_w*.json'): _done.update(json.load(open(_f)))
for D in (5, 10):
    for g in ('0.85', '0.9', '0.95'):
        for dec in ('4000', '8000', '12000'):
            for sd in (42, 43):
                nm = f'R0d{D}e{dec}g{g}_Adam_s{sd}'
                if nm in _done: continue
                jobs.append((nm, AG['Adam'][0], 'adam', sd, dict(STAGE, **COMMON, GAMMA=g, EPS_DECAY=dec, HOVER_DWELL=str(D))))
mine = jobs[WID::NW]; out = {}
try: out = json.load(open(f'{N}/v15a_w{WID}.json'))
...
for name, env, ag, sd, extra in mine:
    ...
    json.dump(out, open(f'{N}/v15a_w{WID}.json', 'w'), default=float)
open(f'{N}/V15a_w{WID}_DONE', 'w').close(); print(f'[w{WID}] 완료', flush=True)
