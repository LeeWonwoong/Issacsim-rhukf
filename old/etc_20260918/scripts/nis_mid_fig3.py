...
"""nis_mid_fig — 중간 필터 M(Q_gyro 1e-2·R_gyro 0.1·Q_euler 5e-4·R_vel 0.1) 을 E·G2 와 겹친 대표 time-step NIS 그림 6패널.
   ...
   출력: results/claudecodefortest/night/nis_mid_compare3.png"""
...
A = 4.36; ATK0 = 180
CFGS = {'E': dict(), 'M': dict(qg=2e-3, rg=0.5, qe=5e-4, rv=0.1), 'M2': dict(qg=2e-3, rg=1.0, qe=5e-4, rv=0.1), 'G2': dict(qg=2e-3, rg=0.2, qe=5e-4, rv=0.1)}
NM = {'E':'E','M':'G3','M2':'G4','G2':'G2'}
...
fig.suptitle('time-step NIS — E(가는 실선) · G2(점선) · G3=Qg2e-3·Rg0.5(굵은 실선) · G4=Qg2e-3·Rg1.0(일점쇄선), 둘 다 Qe5e-4·Rv0.1 · gyro 빨강 / vel 파랑 · 빨간 띠=공격 ON 3 s · 세로 점선=추락')
fig.tight_layout(); os.makedirs('results/claudecodefortest/night', exist_ok=True)
fig.savefig('results/claudecodefortest/night/nis_mid_compare3.png', dpi=110); print('saved', flush=True)
