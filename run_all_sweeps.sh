...
#
# - 시작 시 results_{combined,torque,thrust} 를 비우고 '처음부터' 3개 다 실행.
# - 각 python을 setsid 독립 세션에서 실행 → 한 모드가 끝나도 스크립트 안 죽음.
# 결과: results_{combined,torque,thrust}/ 에 sweep_*.csv + aggregate.txt + run.log
# ============================================================
set -u
PY=python3
echo "########## SWEEP ALL (FRESH) 시작: $(date +%Y%m%d_%H%M%S) ##########"

# ── 처음부터: 기존 결과 폴더 삭제 ──
for m in combined torque thrust; do
    if [ -d "results_${m}" ]; then
        echo " [clean] results_${m} 삭제"
        rm -rf "results_${m}"
    fi
done
