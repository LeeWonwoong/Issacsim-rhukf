...
for s in 42 43 44; do
  ...
  if [ ! -f "$OUT/RUN_DONE" ]; then
    bash scripts/isaac_run.sh "$OUT" --config configs/isaac_v4_swirl.yaml --config configs/overlays/isaac.yaml --set run.seed=$s --set run.outdir=$OUT
  ...
  if [ ! -f "$OUT/RUN_DONE" ]; then
    ...
    bash scripts/isaac_run.sh "$OUT" --config configs/isaac_v4_adam.yaml --config configs/overlays/isaac.yaml --set run.seed=$s --set run.outdir=$OUT
