...
```
  1단계  프로펠러 제거 지상확인                    ✅ 통과
  2단계  Isaac Sim 조종기 수동조종 리허설          ✅ 통과 (2026-08-04)
  3단계  실외 비행 (F1 → F2 → F3)                 ⬜ 다음
         계수 추출. FIELD_PROCEDURE.txt
```

실기 FC DDS 토픽 추가: ✅ 완료 (v1.15.4 + thrust/torque setpoint, 플래시 확인)

### 2단계 결과 (2026-08-04, Isaac SITL v1.18)

| 항목 | 결과 |
|---|---|
| F1 호버 | ✅ 통과 |
| F2 doublet | ✅ 축순서 roll→pitch→yaw 정확, **고도손실 0.01 m** (판정기준 0.2 m) |
| F3 직선왕복 | ✅ 속도 setpoint 경로 최초 검증 |
| 접미사 자동감지 | ✅ 실 PX4 상대 `_v4`/`_v1` 정확 해석 |

**F3 속도 추종** (`watch_offboard.py --csv`, 415 표본):

| 구간 | 중앙 | 90pct | 최대 |
|---|---|---|---|
| 전체(가감속 포함) | 0.082 | 1.147 | 1.532 m/s |
| **정상상태**(명령변경 1.0s 후) | **0.059** | **0.130** | **0.213 m/s** |

명령 변경 후 오차 감쇠: 1.225 → 0.362 → 0.107 → 0.057 → 0.045 m/s
(0.5s 간격). **단조 감소·진동 없음·발산 없음** = 추종 건강.

**제어입력 u**: thrust 중앙 **0.5272** — 캘리브레이션 호버값 0.5274 와 **0.04% 일치**.
플랜트 정합(질량 1.372 kg, C_thrust 25.58)이 독립 경로로 재확인됨.
|τx| 중앙 0.0076 / |τy| 0.0203 / |τz| 0.0005.

⚠ **실외 F3 는 `--leg` 를 늘릴 것.** 정착에 약 1.5 s 가 걸린다.
`v=1.5, leg=3` 이면 기대 등속 2.0 s 인데 실측 정착 1.5 s 를 빼면 쓸 수 있는
구간이 1.5 s 로 줄어 항력 표본이 ~25% 적어진다. 공간이 되면 `--leg 4`.
